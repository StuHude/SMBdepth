...
#from dataloader import SeqDataLoader  XYC瞎改的
...
def log_images(img, depth, pred, args, step):
    ...
    pred = colorize(pred.clone(), vmin=args.min_depth, vmax=args.max_depth)
    wandb.log(
        {
        ...
        for s in args.scales:
            for frame_id in args.frame_ids:
                writer.add_image(
                    ...
                    inputs[("color_tensor", frame_id)][j].data, step)
            ...
            writer.add_image(
                "disp_{}/{}".format(s, j),
                normalize_image(outputs["predict_depth"][j]), step)

            if args.predictive_mask:
                for f_idx, frame_id in enumerate(args.frame_ids[1:]):
                    writer.add_image(
                        "predictive_mask_{}_{}/{}".format(frame_id, s, j),
                        outputs["predictive_mask"]["predict_depth"][j, f_idx][None, ...],
                        step)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--split",type=str,help="which training split to use",
                                  choices=["eigen_tang","eigen_zhou", "eigen_full", "odom", "benchmark"],default="eigen_1")
    ...
    parser.add_argument("--model_name",
                                 ...
                                 default="adabins")
    parser.add_argument("--pretrained_path",type=str,help="the param of pretrained model path",default="./UnetAdaptiveBins__best") 
    ...
    UnetAdaptiveBins_path = args.pretrained_path + "/UnetAdaptiveBins.pth"
    model['UnetAdaptiveBins'],optimizer_state_dict,epoch = model_io.load_checkpoint(UnetAdaptiveBins_path,model['UnetAdaptiveBins'])
    ...
    for epoch in range(args.epoch, epochs):
        ...
        for i, batch in tqdm(enumerate(train_loader), desc=f"Epoch: {epoch + 1}/{epochs}. Loop: Train",
            ...
            # imshow(img,"0")
            depth = batch['depth_gt'].to(device)
            # depthshow(depth,titie="depth_gt")
            bin_edges, pred = model["UnetAdaptiveBins"](img)
            # depthshow(pred,titie="pred")
            ...
            for i,frame_id in enumerate(args.frame_ids[1:]):
                ...
                # imshow(outputs[("color", frame_id)],frame_id)
                if not args.disable_automasking:
            ...
            # log_images(img, depth, pred_log, args, step)
            optimizer.step()
