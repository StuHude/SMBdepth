...
#     return img
def create_mask( c: int, height: int, width: int, border_radius: int, device=None):
...
def predict_pose(args,inputs,model):
    ...
    if args.num_pose_frames == 2:
        ...
        pose_feats = {f_i: inputs["color", f_i] for f_i in args.matching_ids}
        with torch.no_grad():
            # compute pose from 0->-1, -1->-2, -2->-3 etc and multiply to find 0->-3
            for fi in args.matching_ids[1:]:
                if fi < 0:
...
def compute_unsupervied_loss(args,inputs,outputs):
    ...
    if not args.disable_automasking:
        ...
        reprojection_loss_mask = (reprojection_loss_mask *outputs['consistency_mask_adabins'].unsqueeze(1))
    ...
    loss += 0.85 * to_optimise.mean() + 0.15 * reproject_loss 
...
def generate_images_pred(args, inputs, outputs, is_multi=False):
    ...
    for scale in args.scales:
        ...
        for i, frame_id in enumerate(args.frame_ids[1:]):
            ...
            cam_points = backproject_depth_mono(
                depth, inputs[("inv_K")])
            pix_coords = project_3d_mono(
...
def compute_losses(args, inputs, outputs,device ,is_multi=False):
    ...
    disp = outputs[("disp", scale)]
    color = inputs[("color_resize", 0)]
    target = inputs[("color_resize", 0)]
    ...
    if not args.disable_automasking:
        identity_reprojection_losses = []
        for frame_id in args.frame_ids[1:]:
            inputs[("color_resize", frame_id)] = nn.functional.interpolate(inputs[("color", frame_id)], [args.height,args.width], mode='bilinear', align_corners=True)
            pred = inputs[("color_resize", frame_id)]
            identity_reprojection_losses.append(compute_reprojection_loss(args,pred, target))
    ...
    return loss
def compute_matching_mask(args, outputs,device):
    ...
    return mask[:, 0]
def compute_matching_mask_adabins(args, outputs,device):
    """Generate a mask of where we cannot trust the cost volume, based on the difference
    between the cost volume and the teacher, monocular network"""

    mono_output = outputs["predict_depth_upsample"]
    matching_depth = 1 / outputs['lowest_cost_adabins'].unsqueeze(1).to(device)

    # mask where they differ by a large amount
    mask = ((matching_depth - mono_output) / mono_output) < 1.0
    mask *= ((mono_output - matching_depth) / matching_depth) < 1.0
    return mask[:, 0]
def update_adaptive_depth_bins(args, outputs):
    ...
    min_depth = max(args.min_depth, min_depth * 0.9)
    max_depth = max_depth * 1.1

    args.max_depth_tracker = args.max_depth_tracker * 0.99 + max_depth * 0.01
    args.min_depth_tracker = args.min_depth_tracker * 0.99 + min_depth * 0.01
...
if __name__ == "__main__":
    ...
    parser.add_argument("--models_to_load",nargs="+",type=str,help="models to load",default=["pose_encoder", "pose","encoder","depth","mono_encoder", "mono_depth"])        # "encoder", "depth",
    parser.add_argument("--disparity_smoothness",
                                 type=float,
                                 help="disparity smoothness weight",
                                 default=1e-3)   
    ############
    parser.add_argument("--freeze_teacher_and_pose",action="store_true",help="If set, freeze the weights of the single frame teacher network and pose network.")
    parser.add_argument('--num_matching_frames',help='Sets how many previous frames to load to build the cost volume',type=int,default=1)
    parser.add_argument("--num_depth_bins",type=int,default=96)
    parser.add_argument("--depth_binning",help="defines how the depth bins are constructed for the cost volume. 'linear' is uniformly sampled in depth space,'inverse' is uniformly sampled in inverse depth space",type=str,choices=['linear', 'inverse'],default='linear'),
    parser.add_argument('--height', type=int, help='input height', default=192)
    parser.add_argument('--width', type=int, help='input width', default=640)
    parser.add_argument("--v1_multiscale",help="if set, uses monodepth v1 multiscale",action="store_true")
    parser.add_argument("--no_matching_augmentation",action='store_true',help="If set, will not apply static camera augmentation or zero cost volume augmentation during training")
    ...
    ################
    args.train_teacher_and_pose = not args.freeze_teacher_and_pose
    if args.train_teacher_and_pose:
        print('using adaptive depth binning!')
        args.min_depth_tracker = 0.1
        args.max_depth_tracker = 10.0
    frames_to_load = args.frame_ids.copy()
    args.matching_ids = [0]
    args.matching_ids.append(1)
    for idx in range(-1, -1 - args.num_matching_frames, -1):
        args.matching_ids.append(idx)
    ...
    model = {}
    ###################
    model["encoder"] = networks.ResnetEncoderMatching(
        ...
        depth_binning=args.depth_binning, num_depth_bins=args.num_depth_bins)
    model["depth"] = networks.DepthDecoder(model["encoder"].num_ch_enc, args.scales)

    model["mono_encoder"] = \
        networks.ResnetEncoder(18, args.weights_init == "pretrained")
    model["mono_depth"] = \
        networks.DepthDecoder(model["mono_encoder"].num_ch_enc, args.scales)
    ###################
    ...
    for n in args.models_to_load:
        model[n] = model_io.load_pose_model(args,args.pretrained_path,n,model[n])
    ...
    backproject_depth_cv = BackprojectDepth(args.batch_size, h, w)
    backproject_depth_cv.to(device)

    project_3d_cv = Project3D(args.batch_size, h, w)
    project_3d_cv.to(device)
    ...
    backproject_depth_mono = BackprojectDepth(args.batch_size,args.height,args.width)
    backproject_depth_mono.to(device)

    project_3d_mono = Project3D(args.batch_size,args.height,args.width)
    project_3d_mono.to(device)
    ######################################  load  param  #########################################
    params = []
    ###################################### Logging setup #########################################
    ...
    for epoch in range(args.epoch, epochs):
        ...
        for i, batch in tqdm(enumerate(train_loader), desc=f"Epoch: {epoch + 1}/{epochs}. Loop: Train",
            ...
            if args.train_teacher_and_pose:
                pose_pred = predict_pose(args,batch, model)
            else:
                with torch.no_grad():
                    pose_pred = predict_pose(args,batch, model)
            outputs.update(pose_pred)
            CostVolume_outputs.update(pose_pred)
            mono_outputs.update(pose_pred)
             # grab poses + frames and stack for input to the multi frame network
            relative_poses = [batch[('relative_pose', idx)] for idx in args.matching_ids[1:]]
            relative_poses = torch.stack(relative_poses, 1)

            lookup_frames = [batch[('color', idx)] for idx in args.matching_ids[1:]]
            ...
            if not args.no_matching_augmentation: 
                for batch_idx in range(batch_size):
                    ...
                    if rand_num < 0.25:
                        replace_frames = \
                            [batch[('color', 0)][batch_idx] for _ in args.matching_ids[1:]]
                    ...
                    # skip these frames
                        ...
                        augmentation_mask[batch_idx] += 1
            CostVolume_outputs['augmentation_mask'] = augmentation_mask
            min_depth_bin = args.min_depth_tracker
            max_depth_bin = args.max_depth_tracker
            batch["color_resize", 0] = nn.functional.interpolate(batch["color", 0], [args.height,args.width], mode='bilinear', align_corners=True)
            # single frame path
            if args.train_teacher_and_pose:
                feats = model["mono_encoder"](batch["color_resize", 0])
                mono_outputs.update(model['mono_depth'](feats))
            else:
                with torch.no_grad():
                    feats = model["mono_encoder"](batch["color_resize", 0])
                    mono_outputs.update(model['mono_depth'](feats))
            generate_images_pred(args,batch, mono_outputs)
            mono_losses = compute_losses(args,batch, mono_outputs, device,is_multi=False)
            ...
            for key in list(mono_outputs.keys()):
                ...
                if _key[0] in ['depth', 'disp']:
                    _key[0] = 'mono_' + key[0]
                    _key = tuple(_key)
                    CostVolume_outputs[_key] = mono_outputs[key]
            # multi frame path
            features, lowest_cost, confidence_mask = model["encoder"](batch["color_resize", 0],
                                                                        lookup_frames,
                                                                        relative_poses,
                                                                        batch[('K', 2)],
                                                                        batch[('inv_K', 2)],
            ...
            CostVolume_outputs.update(model["depth"](features))
            CostVolume_outputs["lowest_cost"] = F.interpolate(lowest_cost.unsqueeze(1),[args.height, args.width],mode="nearest")[:, 0]
            CostVolume_outputs["consistency_mask"] = F.interpolate(confidence_mask.unsqueeze(1),[args.height, args.width],mode="nearest")[:, 0]
            outputs["lowest_cost_adabins"] = F.interpolate(lowest_cost.unsqueeze(1),[args.input_height, args.input_width],mode="nearest")[:, 0]
            outputs["consistency_mask_adabins"] = F.interpolate(confidence_mask.unsqueeze(1),[args.input_height, args.input_width],mode="nearest")[:, 0]
            
            if not args.disable_motion_masking:
                        CostVolume_outputs["consistency_mask"] = (CostVolume_outputs["consistency_mask"] *
                                                    compute_matching_mask(args,CostVolume_outputs,device))

            generate_images_pred(args,batch, CostVolume_outputs, is_multi=True)
            CostVolume_loss = compute_losses(args,batch, CostVolume_outputs,device, is_multi=True)
            CostVolume_loss += mono_losses
            ...
            outputs["predict_depth_upsample"] = nn.functional.interpolate(outputs["predict_depth"], depth.shape[-2:], mode='bilinear', align_corners=True)
            outputs["costVolume_depth"] = nn.functional.interpolate(CostVolume_outputs[("depth", 0, 0)], depth.shape[-2:], mode='bilinear', align_corners=True)
            # outputs["mono_depth"] = nn.functional.interpolate(mono_outputs[("depth", 0, 0)], depth.shape[-2:], mode='bilinear', align_corners=True)
            outputs["mono_depth"] = depth

            depths = (1 / torch.linspace(1/args.max_depth, 1/args.min_depth, 32,device=device)).view(1, -1, 1, 1).expand(batch_size, -1, args.height, args.width)
            cost_volumes = []
            single_frame_cvs = [[] for i in range(args.frame_ids[1:])]
            for i,frame_id in enumerate(args.frame_ids[1:]):
                T = outputs["cam_T_cam",0,frame_id]
                # from the authors of https://arxiv.org/abs/1712.00175
                if args.pose_model_type == "posecnn":
                ...
                warped_images = []
                warped_masks = []
                batch_depths = depths[0, :, :, :]
                depth_value_count = batch_depths.shape[0]
                cam_points = torch.cat([cam_points, backproject_depth_cv.ones.expand(depth_value_count, -1, -1)], 1)
                image_to_warp = batch[("color", frame_id)][0, :, :, :].unsqueeze(0).expand(depth_value_count, -1, -1, -1)
                mask_to_warp = create_mask(1, args.height, args.width, 2, device).expand(depth_value_count, -1, -1, -1)
            ...
            difference = torch.abs(warped_images - batch[("color", 0)][0])
            difference = difference.permute(1, 2, 0, 3, 4)
            batch_size, channels, height, width = batch[("color", 0)].shape
            sad_kernel = (
                    torch.tensor(channels * [1 / channels], device=device) / (3 ** 2)) \
                .view(1, channels, 1, 1, 1).repeat(1, 1, 1, 3, 3)
            sad_volume = F.conv3d(difference, sad_kernel, padding=(0, 3 // 2,
                                                                        3 // 2)).squeeze(1)
            single_frame_cv = (1 - sad_volume * 2) * (torch.any(warped_images != 0, dim=2) | torch.all(warped_images == batch[("color", 0)][0], dim=2)).permute(1, 0, 2, 3)
            sum_item = torch.exp(-10 * torch.pow(sad_volume - torch.min(sad_volume, dim=1, keepdim=True)[0], 2))
            ...
            if args.train_teacher_and_pose:
                update_adaptive_depth_bins(args,CostVolume_outputs)
            if not args.disable_motion_masking:
                        outputs["consistency_mask_adabins"] = (outputs["consistency_mask_adabins"] *
                                                    compute_matching_mask_adabins(args,outputs,device))
            ...
            loss = args.w_chamfer * l_chamfer + reproject_loss + CostVolume_loss
            loss = reproject_loss + CostVolume_loss
            losses = {}
            losses["reproject_loss"] = reproject_loss
            losses["l_dense"] = l_dense
            losses["l_chamfer"] = l_chamfer
    ...
    ########################################################################################################
            ...
            if should_write and step % args.validate_every == 0:
                ...
                if should_log:
                    ...
                    model_io.save_checkpoint(args,model, optimizer, epoch, f"{experiment_name}_{run_id}_latest",
                                                root=os.path.join('.', "checkpoints"))

                if metrics['abs_rel'] < best_loss and should_write:
                    model_io.save_checkpoint(args,model, optimizer, epoch, f"{experiment_name}_{run_id}_best.pt",
                                                root=os.path.join('.', "checkpoints"))
